...
test('all new categories preserve local identifiers/enrichment across atomic failure, partial update and resume', async t => {
  ...
  const ftsBefore = db.sqlite.prepare('SELECT rowid,* FROM extended_product_fts ORDER BY rowid').all();
  ...
  assert.deepEqual(db.sqlite.prepare('SELECT rowid,* FROM extended_product_fts ORDER BY rowid').all(), ftsBefore);
