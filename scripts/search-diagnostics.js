...
const {values:args}=parseArgs({options:{port:{type:'string',default:'8788'}}});
...
try {
  ...
  const handle=createSearchDiagnostics({db,epoch:`local-${state.results[0]?.id??'unversioned'}`});
  ...
  console.log(`検索診断: http://127.0.0.1:${server.address().port}`);
