...
console.log(JSON.stringify({ ready: true, worker: config.name, binding: 'DB', database: 'pc-parts-catalog', database_id: id, active: results[2].results[0].active, sync: results[1].results[0], cache_epoch: config.vars.CATALOG_CACHE_EPOCH, ratelimits: config.ratelimits }, null, 2));
