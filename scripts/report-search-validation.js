...
const report={release_gate:releaseGate,snapshot:quality.catalog.last_sync.source_commit,migration:await load('migration'),integrity:await load('integrity'),
  ...
  reviewed_lookup:summarizeUX(quality.results.filter(r=>r.intent==='lookup'&&r.review!=='pending')),
  ...
  quality_failures:qualityFailures(quality,{requireReview:false}),pending_review:quality.summary.pending_review,
