...
export function offerCachePolicy(url, jan, env) {
  ...
  // Provider results are independent of catalog ID/name: share across products with the same JAN.
  ...
  key.search = new URLSearchParams({ epoch: eligible ? epoch : 'unversioned', ttl: String(ttl), strategy: 'jan', identifier: jan }).toString();
